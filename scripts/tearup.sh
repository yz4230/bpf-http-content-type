...
# ns1 <-> [ns2 <-> ns3 <-> ns4] <-> ns5
#                   |
#                  ns6 (VNF; IPv6-only, inside SRv6 segment)
...
ip link add veth3@ns3 type veth peer name veth1@ns6
...
ip link set veth3@ns3 netns ns3
ip link set veth1@ns6 netns ns6
...
ip -n ns3 addr add fd00:3::1/64 dev veth3@ns3
ip -n ns6 addr add fd00:3::2/64 dev veth1@ns6
...
ip -n ns2 -6 route add $SID_NS6_END/128 via fd00:1::2 dev veth2@ns2
...
ip -n ns3 -6 route add $SID_NS6_END/128 via fd00:3::2 dev veth3@ns3
...
ip -n ns4 -6 route add $SID_NS6_END/128 via fd00:2::1 dev veth1@ns4

ip -n ns6 -6 route add $SID_NS2_DX4/128 via fd00:3::1 dev veth1@ns6
ip -n ns6 -6 route add $SID_NS3_END/128 via fd00:3::1 dev veth1@ns6
ip -n ns6 -6 route add $SID_NS4_DX4/128 via fd00:3::1 dev veth1@ns6
...
ip -n ns6 -6 route add $SID_NS6_END/128 encap seg6local action End dev veth1@ns6

# Configure SRv6 encapsulation on edge nodes (IPv4-in-IPv6+SRH).
ip -n ns2 route add 10.0.2.0/24 encap seg6 mode encap segs $SID_NS3_END,$SID_NS6_END,$SID_NS4_DX4 dev veth2@ns2
ip -n ns4 route add 10.0.1.0/24 encap seg6 mode encap segs $SID_NS2_DX4 dev veth1@ns4
